                ...
                <div class="grid lg:grid-cols-2 gap-6 mb-8">
                            ...
                            @foreach($recent_events as $event)
                                <div class="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all">
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-gray-400">{{ \Carbon\Carbon::parse($event->date)->format('M d, Y') }}</p>
                                    </div>
                                    <div class="text-right">
                                        ...
                                        <div class="w-24 h-2 bg-gray-700 rounded-full overflow-hidden">
                                            <div class="progress-bar h-full bg-gradient-to-r from-blue-600 to-blue-400" style="width: {{ $event->event_total_tickets ? round((($event->tickets_sold ?? 0) / $event->event_total_tickets) * 100) : 0 }}%"></div>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <a href="{{ route('admin.sales') }}" class="text-blue-400 hover:text-blue-300 text-sm font-medium">View All</a>
                        </div>
                        <div class="space-y-4">
                            @foreach($recent_sales as $sale)
                                <div class="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all">
                                    ...
                                    <div class="text-right">
                                        <p class="font-semibold text-white mb-1">₱{{ number_format($sale->total_amount, 2) }}</p>
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $sale->status == 1 ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400' }}">
                ...
                <div class="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <button class="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-blue-600/20 to-blue-400/10 border border-blue-500/30 rounded-xl hover:border-blue-500/50 transition-all group">
                            ...
                            <span class="text-white font-semibold">Control Panel</span>
                        </button>
