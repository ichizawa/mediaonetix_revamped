<aside id="sidebar"
    class="fixed top-0 left-0 h-screen w-64 bg-gradient-to-b from-[#111827] to-[#0c1222] border-r border-white/10 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 z-50 overflow-y-auto">
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 p-6 space-y-2 overflow-y-auto">
            ...
            {{-- @php
                $prefix = auth()->user()->routePrefix();
            @endphp
            @forelse($sidebars as $sb)
                <a href="{{ route($prefix . '.' . $sb->type) }}"
                    class="nav-link flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs(auth()->user()->routePrefix() . '.' . $sb->type . '*') ? 'bg-blue-500/20 border border-blue-500/30 text-blue-400' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z">
                        </path>
                    </svg>
                    <span class="font-medium">{{ $sb->name }}</span>
                </a>
            @empty
                <a href="#">
                    <span class="font-medium">No Sidebar</span>
                </a>
            @endforelse --}}
            ...
            @if (auth()->user()->isMerchant() || auth()->user()->isAdmin())
                ...
                @if (auth()->user()->isMerchant())
                    <a href="{{ route(auth()->user()->routePrefix() . '.organizers') }}"
                        class="nav-link flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs(auth()->user()->routePrefix() . '.organizers*') ? 'bg-blue-500/20 border border-blue-500/30 text-blue-400' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z">
                            </path>
                ...
                @if (auth()->user()->isUser())
                    <a href="{{ route(auth()->user()->routePrefix() . '.purchase-history') }}"
                        class="nav-link flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs(auth()->user()->routePrefix() . '.control-panel*') ? 'bg-blue-500/20 border border-blue-500/30 text-blue-400' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6 space-y-2 border-t border-white/10">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-red-500/10 text-gray-400 hover:text-red-400 transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<div id="sidebarOverlay" onclick="toggleSidebar()"
    class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 lg:hidden hidden"></div>
