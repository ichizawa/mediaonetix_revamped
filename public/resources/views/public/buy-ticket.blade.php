...
@section('content')
    ...
    <div class="container py-5" style="padding-left: 20px !important; padding-right: 20px !important;">
        <div class="row">
                ...
                <form action="{{ route('create.sale') }}" method="POST" id="form_1">
                    ...
                    <div class="d-none" id="step-2">
                        ...
                        <div class="card shadow-sm p-4 payment-method-card">
                            <h5 class="fw-bold">Pay with QR PH</h5>
                            <p class="text-muted">Scan the code on the next screen using your preferred bank or e-wallet
                                app.</p>
                                ...
                                @php
                                    ...
                                    foreach ($processors as $processor) {
                                        if (Str::startsWith($processor['procId'], 'ABQR')) {
                                            $qrPhProcessor = $processor;
                                            break;
                                        }
                                    }
                                @endphp
                                @if ($qrPhProcessor)
                                    <div id="payment_form" style="height: auto;">
                                        <div class="card shadow-md payment-method-card-option selected w-100 p-3" style="justify-content: center; align-self: center;"
                                            data-value="{{ $qrPhProcessor['procId'] }}">
                                            <div class="d-flex align-items-center">
                                                <img width="80" class="me-3" src="{{ $qrPhProcessor['logo'] }}"
                                                    alt="{{ $qrPhProcessor['shortName'] }}" />

                                                <div class="flex-grow-1">
                                                    <span class="fs-6 fw-bold">{{ $qrPhProcessor['shortName'] }}</span>
                                                </div>
                                        ...
                                        <input type="hidden" name="payment_method" id="payment_method_input"
                                            value="{{ $qrPhProcessor['procId'] }}" />
                                    </div>
                                @else
                                    <div class="alert alert-warning" role="alert">
                                        QR PH payment method is currently unavailable.
                                    </div>
                                @endif
                            </div>

                            <div class="d-flex justify-content-between gap-2 mt-4">
                                <button type="button" class="btn btn-dark flex-grow-1" id="prev-2">Back</button>
